...
if [[ "${1:-}" == "--integration" ]]; then
    ...
    cc scripts/stock-test-input.c build/fake-input-protocol.c -Ibuild -lwayland-client -lm -o build/stock-test-input
    python scripts/run-stock-plugin.py --headless --check
elif [[ $# -gt 0 ]]; then
